...
const App = () => {
  const [historial, setHistorial] = useState(() => {
    try {
      const saved = localStorage.getItem('tracker_data_v3'); // Bump version to v3 for the new field
  ...
  return (
    <div className="min-h-screen bg-[#1e1f22] flex flex-col xl:flex-row items-center xl:items-stretch justify-center p-6 gap-8 font-sans selection:bg-emerald-500/30 text-slate-200">
      
      {/* 1. IZQUIERDA: Productividad */}
      <div className="max-w-md w-full bg-[#2b2d31] rounded-[2.5rem] shadow-[0_30px_60px_rgba(0,0,0,0.6)] border border-white/10 overflow-hidden relative flex flex-col">
        ...
        <div className="p-8 space-y-4 flex-1">
          ...
          <div className="bg-gradient-to-br from-rose-500/10 to-transparent border border-rose-500/20 p-5 rounded-3xl flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button onClick={() => updateStat('tiempoOcio', -0.25)} className="p-2 bg-white/5 hover:bg-white/10 rounded-lg border border-white/5"><Minus size={16} /></button>
      ...
      {/* 2. CENTRO: Nutrición */}
      <div className="max-w-md w-full bg-[#2b2d31] rounded-[2.5rem] shadow-[0_30px_60px_rgba(0,0,0,0.6)] border border-white/10 overflow-hidden relative">
            ...
            <div className="grid grid-cols-1 gap-3 mb-4">
              ...
              <div className={`transition-all duration-500 p-4 rounded-3xl border flex flex-col gap-3 relative overflow-hidden ${
                ...
                <div className="flex gap-1 justify-between items-center bg-black/20 p-2 rounded-xl mt-1">
                  ...
                  {historial.length < 14 && [...Array(14 - historial.length)].map((_, i) => (
                    <div key={i + historial.length} className="w-2 h-2 rounded-full bg-white/5 opacity-20" />
                  ))}
                </div>
              </div>
            </div>
      ...
      {/* 3. DERECHA: Objetivo Peso & Gráfico */}
      <div className="max-w-sm w-full bg-[#2b2d31] rounded-[2.5rem] shadow-[0_30px_60px_rgba(0,0,0,0.6)] border border-white/10 p-8 flex flex-col items-center relative overflow-hidden">
